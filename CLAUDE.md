...
- **Build the manuscript `.docx` with the `manuscript-docx` skill** (global, reusable across projects — scripts + reference template live there, not in this repo). It produces `manuscript/draft.docx` with manuscript-review formatting **and live, refreshable Zotero citation fields** — open it in Word, click Zotero → **Refresh** (pick **AMA** on first run), then **Add/Edit Bibliography** to insert the reference list. Invoke the skill, or run it directly:
  ```
  bash ~/.claude/skills/manuscript-docx/scripts/build-docx.sh \
    --draft manuscript/draft.md --bib references.bib --keymap manuscript/zotero-item-keys.json
  ```
  - **`manuscript/zotero-item-keys.json`** (project-specific) maps Better BibTeX citekeys → Zotero item keys (library 5085299, collection **BSDVTKC7**). Regenerate via the Zotero MCP (`zotero_get_collection_items BSDVTKC7`) whenever citations are added or renamed; the build errors and names any missing key. **New Introduction/Discussion citations must be added here before the build resolves them.**
  - The skill bundles the reference template and a static-preview command; see its `SKILL.md` for the formatting spec and troubleshooting. The generic `pandoc-reference.docx` in this repo is kept only for non-manuscript outputs.
