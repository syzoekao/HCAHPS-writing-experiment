...
Direct PDF retrieval of the OQR Specifications Manual pages for the 2017 and 2024 vintages was blocked at validation time (sandboxed `WebFetch` permission denied), so the diff below is reconstructed from secondary CMS and program-information sources rather than from cover-page-to-cover-page comparison. The version-history checks needed (numerator, denominator, exclusions, time-stamp methodology, "Not Available"/"Too few cases" coding, sampling rules) are still **TBD-direct-PDF-diff** and should be confirmed by the analyst against the QualityNet archive before the manuscript Methods section is finalized.

What can be established from secondary sources:

- **Family of measures.** The OP-18 family has four stratifications: OP-18a (overall — all-patient rate), OP-18b (discharged ED patients, excluding psychiatric/mental health and transfer cases), OP-18c (psychiatric/mental health patients), OP-18d (transfer patients). Sources: QualityNet Hospital OQR Specifications Manual; American Hospital Directory measure documentation (https://www.ahd.com/definitions/qual_hqi_measurements.html?conditionid=18); CMS ED-Throughput stratification table (https://www.cms.gov/medicare/quality-initiatives-patient-assessment-instruments/qualitymeasures/downloads/eh_edthroughputstratificationtable.pdf). The "all patients, including psychiatric/mental health and transfer" plain-language description matches **OP-18a**.
- **Numerator / denominator / time-stamp methodology.** Across all OP-18 variants, the measure value in minutes is *ED Departure Date and Time minus Outpatient Encounter Date and Arrival Time*, hospital-median over the relevant population. This calculation has not changed in any OQR specifications-manual release the secondary sources flag. Source: OP-18 fact sheet (https://worh.org/wp-content/uploads/2016/06/MBQIP-Measures-Fact-Sheets-OP18.pdf); Hospital OQR Specifications Manual v10.0 ED-Throughput set (https://test.rwhc.com/ProjectTracker/KnowledgeBaseAttachments/46_1d_ED_Throughput_set_v10.0.pdf).
- **Public-reporting status.** OP-18b (discharged-only) is the *currently active and publicly reported* OQR ED-throughput measure as of the 2024 manual version (v19.0; CMS Hospital OQR Specifications Manual v19.0, https://www.reginfo.gov/public/do/DownloadDocument?objectID=165405700). It is scheduled for removal beginning with the CY 2028 reporting period and replacement by an electronic clinical quality measure (Emergency Care Access & Timeliness, https://ecqi.healthit.gov/ecqm/hosp-outpt/2027/cms1244v1).
- **Sampling rules.** Population-and-sampling specifications for OP-18 (Hospital OQR Specifications Manual Appendix A, version 10 and later) allow hospitals to submit either their entire ED population or a simple/systematic random sample; sample-size requirements use a 10% margin-of-error rule (e.g., 63 cases per quarter for populations ≤900, 96 for >900). The sample-size table has had small revisions across manual versions but no documented change that would mechanically halve a hospital's reported median.
- **"Not Available" / "Too few cases" coding.** OP-18 uses suppression rules tied to denominator size; the Stata pipeline in [stata/do.do](stata/do%202.do) recodes "Not Available" strings to missing before destringing.
- **The likely mechanical change is the all-patient → discharged-only narrowing.** The most consequential change relevant to the 292→181 minute drop is that the **OP-18a "all-patient overall rate" stratification was phased out of routine public reporting** during the 2018–2020 window, with the surviving publicly reported measure being **OP-18b (discharged only, excluding psychiatric/mental health and transfer)**. Psychiatric and transfer ED visits are the long-tail observations that drive the OP-18a hospital median materially higher than the OP-18b hospital median. If the Care Compare 2024 *Timely and Effective Care – Hospital* file still carries the literal "all patients" plain-language description in its `Measure Name` column but the underlying population is now the OP-18b (discharged-only) population, then a string-match filter — as in [stata/do.do](stata/do%202.do) line 22 — would unknowingly capture a re-scoped measure across the two vintages.

This OP-18a → OP-18b scope narrowing is the candidate (b) "Measure-definition change" mechanism in [§8 flag 3](#8-notes-and-flags) and is the most likely single explanation for the magnitude of the observed drop. **Direct verification against the 2017 vs 2024 OQR Specifications Manual cover pages — including release notes for v10.0 (2017), v13.0 (2020), and v19.0 (2024) — remains TBD-direct-PDF-diff and should be the first item the analyst checks against the QualityNet archive (https://qualitynet.cms.gov/outpatient/specifications-manuals).**
...
4. **Direct OQR Specifications Manual cover-page diff (2017 v10.0, 2020 v13.0, 2024 v19.0) is still TBD-direct-PDF-diff** and should be the first item the analyst confirms against the QualityNet archive. The verdict above will likely strengthen rather than reverse if the diff is completed, but a manual confirmation would let the manuscript cite the version numbers and release notes specifically.
